...
function atFromOptions(options) {
	...
	if (util.isString(options.at)) {
		...
		case "end": return -1; 
		default:
			util.assert(false, "at: unsupported string value, got ''{0}''", options.at);
		}
	} else if (util.isNumber(options.at)) {
		util.assert(options.at >= 0, "at: expected number >= 0, got {0}", options.at);
		return options.at;
	} else {
 ...
 * 
 * <p>If no parameters are passed in, the action is registered with its toolbar id (see property tooblarId)
 * for being displayed in the toolbar. It isn't displayed, however, unless the toolbar id is already included
 * in the toolbar preferences. If not, the user first have to configure it manually in the preference dialog.</p>
 * 
 * <p>Use one of the named options described below, if you want to enforce, that the action is displayed at a
 * a specific position in the toolbar. If you use one of these options, the toolbar configuration is saved 
 ...
 *   <dd>Display the toolbar entry at a specific position, at the start or at the end of the toolbar.</dd>
 *   
 *   <dt><code class="signature">after: string</code> </dt>
 *   <dd>Display the toolbar entry after the entry with toolbar id <code>after</code>.</dd>
 *
 *   <dt><code class="signature">before: string</code> </dt>
 *   <dd>Display the toolbar entry before the entry with toolbar id <code>before</code>.</dd>
 * </dl> 
 * 
 * @example 
 * var JSAction = require("josm/ui/menu").JSAction;
...
mixin.addToToolbar = function(options) {
	...
	var toolbarPrefs = new ArrayList(Main.pref.getCollection("toolbar",new ArrayList()));
	
	// The following is clumsy. We have to fiddle with preference settings in order
	// to display a toolbar entry at a specific position
	//	
