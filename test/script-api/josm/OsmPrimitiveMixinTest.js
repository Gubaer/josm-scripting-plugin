...
suites.push(tu.suite("properties access",
    ...
    tu.test("property 'user'", function() {
        ...
        tu.expectAssertionError(
            "user - can't assign unkonwn user name", function() {
            n.user = "nosuchuser";
        });

        tu.expectAssertionError(
            "user - can't assign unkonwn user id", function() {
            n.user = 9999;
        });

        tu.expectAssertionError(
            "user - can't assign an object", function() {
            n.user = {name: "test"};
        });
    }),
...
suites.push(tu.suite("setting and getting tags",
    ...
    tu.test("remove a tag - using tags object - using delete opeartor", 
        function() {
        var n = nb.create(1234);
    ...
    tu.test("assign tags - an object with properties", function() {
        ...
        util.assert(n.get("name") == "aname", "1 - tag should be <aname>");
        util.assert(n.get("normalized") == " a value with blanks ", 
            "1 - tag should be < a value with blanks >");
    }),
