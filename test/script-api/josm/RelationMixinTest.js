...
suites.push(tu.suite("properties access",
    ...
    test("members - read", function() {
        ...
        util.assert(r.members.length == 0,
            "should be empty array, for proxy relation too");
    ...
    test("members - assign array or list ", function() {
        ...
        r = rb.create(1234);
        r.members = [nb.create(), rb.member('role.1', nb.create()),
                     wb.create(12345)];
        ...
        r = rb.create(1234);
        r.members = [null, nb.create(), rb.member('role.1', nb.create()),
                     undefined, wb.create(12345)];
...
suites.push(tu.suite("get and set relation members using [...]",
    test("get with [..]", function() {
        var members = [nb.create(), rb.member('role.1', nb.create()),
                       wb.create(12345)];
    ...
    test("set with [..]", function() {
        ...
        util.assert(r[1].role == "role.2",
            "unexpected role, should be {0}", "role.2");
        r[3] = nb.create();
