...
public class JSMixinRegistry {
    ...
    /**
     * Replies the mixin for the class <code>cls</code>, or null, if no mixin
     * for this class is available.
     *
     * @param cls the class
     * @return the mixin
     */
     ...
     */
    static public void loadJSMixin(Scriptable parentScope, String moduleName)
            throws JSMixinException{
        Scriptable module;
        try {
            module = RhinoEngine.getInstance().require(moduleName);
        } catch(RhinoException e){
            throw JSMixinException.make(e, "Failed to load module ''{0}'' "
                    + "providing a JS mixin.",moduleName);
        }
        Object forClass = module.get("forClass", module);
        if (forClass == Scriptable.NOT_FOUND) {
            throw JSMixinException.make("Property ''{0}'' in module ''{1}'' "
                    + "not found.", "forClass", moduleName);
        }
        forClass = unwrap(forClass);
        if (! (forClass instanceof Class<?>)) {
            throw JSMixinException.make(
                    "Unexpected value for property ''{0}'' in module ''{1}''. "
                    + "Expected a java class object, got {2}",
                    "forClass", moduleName, forClass);
        }
        ...
        if (mixin == Scriptable.NOT_FOUND) {
            throw JSMixinException.make(
                    "Property ''{0}'' in module ''{1}'' not found.",
                    "mixin", moduleName);
        }
        if (! (mixin instanceof Scriptable)) {
            throw JSMixinException.make(
                    "Unexpected value for mixin ''mixin'' in module ''{0}''. "
                    + "Expected a Scriptable, got {1}",
                    "mixin", moduleName, mixin);
        }
