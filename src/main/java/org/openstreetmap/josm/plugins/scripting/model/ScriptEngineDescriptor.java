...
public class ScriptEngineDescriptor implements PreferenceKeys {
     ...
     * @param engineId   the engine id. Must not be null.
     * @throws NullPointerException if <code>engineType</code> or <code>engineId</code> is null
     */
    public ScriptEngineDescriptor(@NotNull ScriptEngineType engineType, @NotNull String engineId) {
    ...
    /**
     * Replies the descriptor for the default scripting engine.
     * <p>
     ...
     */
    static public @Null ScriptEngineDescriptor buildDefaultFromPreferences(@Null final Preferences preferences) {
        if (preferences == null) {
            return null;
        }
     ...
     *
     * @param preferenceValue the preference value. Must not be null.
     * @return the scripting engine descriptor. null, if <code>preferenceValue</code> doesn't
     * refer to a supported scripting engine
     * @throws NullPointerException if <code>preferenceValue</code> is null
     */
    static public @Null ScriptEngineDescriptor buildFromPreferences(@NotNull final String preferenceValue) {
        Objects.requireNonNull(preferenceValue);
        ...
        switch (type) {
            case PLUGGED:
                // don't lowercase. Lookup in ScriptEngineManager could be case-sensitive
                engineId = engineId.trim();
                if (JSR223ScriptEngineProvider.getInstance().getEngineById(engineId) == null) {
            ...
            default:
                ...
                throw new IllegalStateException(format(
                    "unexpected scripting engine type ''{0}''", type
                ));
        }
    }

    protected void initParametersForJSR223Engine(String engineId) {
        final var factory = JSR223ScriptEngineProvider.getInstance().getScriptFactoryByName(engineId);
        initParametersForJSR223Engine(factory);
    }
    ...
    public @NotNull String getFullEngineId() {
        return engineType.preferencesValue + "/" + engineId;
    }
     ...
     * @return engine type
     */
    public @NotNull ScriptEngineType getEngineType() {
        return engineType;
    }
     ...
     */
    public @NotNull String getEngineId() {
        return engineId;
    }
    ...
    public enum ScriptEngineType {
        ...
        /**
         * Infers the script engine type from a preference value. The value is
         ...
         * @return the type
         */
        static public @Null ScriptEngineType fromPreferencesValue(@NotNull String preferencesValue) {
            Objects.requireNonNull(preferencesValue);
