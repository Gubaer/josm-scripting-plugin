...
const ressourcesDir = `${josmScriptingPluginHome()}/src/test/resources/sample-data-files`

suites.push(suite('load',
  test('can load an OSM file', () => {
    const path = `${ressourcesDir}/test-datasetutil-load.osm`
  ...
  test('can load an OSM file with a non-standard suffix', () => {
    const path = `${ressourcesDir}/test-datasetutil-load.osm-unknown-suffix`
    const dsutil = DataSetUtil.load(path, { format: 'osm' })
    util.assert(dsutil.node(1) !== null, 'node 1 should have been loaded')
  }),

  test('can load an OSC file', () => {
    const path = `${ressourcesDir}/test-datasetutil-load.osc`
    const dsutil = DataSetUtil.load(path)
  ...
  test('can load an OSC file with an unknown suffix', () => {
    const path = `${ressourcesDir}/test-datasetutil-load.osc-unknown-suffix`
  ...
  test('can load a compressed OSM file', () => {
    const path = `${ressourcesDir}/test-datasetutil-load.osm.gz`
    const dsutil = DataSetUtil.load(path)
    util.assert(dsutil.node(1) !== null, 'node 1 should have been loaded')
  }),

  test('can load a compressed OSM file with a non-standard suffix', () => {
    const path = `${ressourcesDir}/test-datasetutil-load.osm.gz-unknown-suffix`
