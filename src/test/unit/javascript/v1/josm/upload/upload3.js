(function() {
/** 
 * Functional test - uploads a data set in chunked mode, in two steps, using the
 * same changeset
 */
...
for(var i=0; i< 50; i++) {
	data.add(nb.withPosition(46.9488598, 7.4627158).create());
}
// uploading 50 nodes in already open changeset, in chunked mode, 10 objects
// per request, without closing the changeset after the upload
//
api.upload(data, "test upload - 1", {
	strategy: "chunked",
	changeset: cs,
	chunkSize: 10,
