...
import {AddCommand, add} from 'josm/command'
...
suites.push(suite('add',
  ...
  test('add - arguments with nulls, undefined, and doublettes', function () {
    const n1 = NodeBuilder.create()
    const w1 = WayBuilder.create()
    const cmd = add(n1, null, w1, undefined, n1)
    util.assert(cmd instanceof AddCommand, 'wrong type')
    util.assert(cmd._objs.length === 2, 'wrong length')
  }),
  ...
  test('add - mixed arguments', function () {
    ...
    const w1 = WayBuilder.create()
    const list = new ArrayList()
    list.add(NodeBuilder.create()); list.add(NodeBuilder.create())
    const cmd = add(n1, [n1, w1], null, [list, n1, undefined])
    util.assert(cmd instanceof AddCommand, 'wrong type')
